...
export default function AdminStaff() {
  ...
  const updateMutation = trpc.staffDirectory.update.useMutation({
    onSuccess: () => utils.staffDirectory.list.invalidate(),
    onError: (error) => showToast(getErrorMessage(error), "error"),
  });
      ...
      <ScrollView contentContainerClassName="gap-4 p-4 pb-8" keyboardShouldPersistTaps="handled">
        ...
        {showForm && (
          <View className="gap-3 rounded-lg border border-border-subtle bg-surface p-4">
            ...
            <Input label="Category" placeholder="e.g. plumber, electrician, cook" value={category} onChangeText={setCategory} />
            <Input label="Contact Number" placeholder="+91XXXXXXXXXX" keyboardType="phone-pad" value={phone} onChangeText={setPhone} />
          ...
          <View className="gap-2">
            {staff.map((entry) => (
              <View key={entry.id} className="flex-row items-center gap-3 rounded-xl border border-border-subtle bg-surface p-4">
                ...
                <Pressable onPress={() => confirmDelete(entry.id, entry.name)} hitSlop={8} className="p-1">
