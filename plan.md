...
**Phase 11 is fully done and fully verified on-device** (2026-07-16) — the one remaining gap from the initial pass (guard Entry & Exit History and Resident Directory screens, which had only been code-reviewed, not eyeballed) was explicitly checked and confirmed by the user. Only two things remain, both disclosed and deliberate: (1) the commit, held back per standing instruction until explicitly requested, and (2) a future EAS rebuild to activate haptics/splash on-device.

### Post-Phase-11 addendum: Impeccable-driven UI/UX audit and fixes
After Phase 11 closed, the user asked for help maximizing the app's UI/UX for a hackathon whose rubric explicitly weights visual design. Set up `apps/mobile/.agents/skills/impeccable` (already installed by the user, not part of the original plan) — wrote `PRODUCT.md`, deliberately skipped the web-oriented `DESIGN.md`/live-mode machinery since this is a native app, and ran a dual-assessment critique (independent design review + independent code-level structural scan) against representative screens across all 3 roles. Explicit call made up front: preserve the existing Linear-inspired design system as locked brand identity, use Impeccable evaluatively rather than letting it push its own Material-3-leaning opinions.

Found and fixed 5 real issues, snapshot at `apps/mobile/.impeccable/critique/2026-07-15T19-05-30Z__apps-mobile-app-whole-app-cross-role.md`:
- **P0 (real bug):** `KeyboardAvoidingView` resolved to no-op (`undefined`) on Android — the app's only shipping platform — in 4 screens. Fixed to `behavior="height"` on Android.
- **P1:** two undocumented colors (`#e5484d`, `#3dd68c`, 8 sites) bypassing the documented `status-red`/`status-green` tokens — fixed to the documented values. The `rounded-xl` (12px) radius, found in wide consistent use across 21 sites but never named, was formalized as a documented third radius token instead of mass-changed.
- **P1:** the same visitor status rendered as "Waiting" in some screens and "Pending" in others (a 3rd, raw-lowercase rendering also found on the admin dashboard) — centralized into `lib/visitor-status.ts`, all 4 consumers migrated.
- **P1:** confirm/undo protection was inconsistent by screen, not by actual risk. Resolved with an explicit risk-based policy after discovering there's no backend endpoint to reverse `markEntry`/`markExit` (the visitor state machine is deliberately one-directional, backed by an append-only audit ledger — a real Phase 4 design constraint, not something to route around silently): Reject now has a blocking confirm; Mark Entry/Exit and Activate/Confirm-Move stay one-tap, per the user's explicit call, since real undo isn't feasible without new backend work and a confirm would slow down the guard's highest-frequency action.
- **P2:** the 9 admin CRUD screens were 9 hand-written copies of the same list/form scaffold. Extracted `FormPanel`, `ListRowCard`, `IconButton`, `ListLoading` (new `components/ui/`) and migrated all 9 screens plus a 14-file app-wide sweep for the duplicated loading spinner.

All fixes verified: root `pnpm check-types` clean, `expo export --platform web` clean, no backend changes (pure frontend pass, no reseed needed).
